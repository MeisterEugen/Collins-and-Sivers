...
int main(int argc, char* argv[]) {
  ...
  file_name.append("HERMES/zSivers_");
  ...
  pythia.readString("Beams:idA = 11");      // BeamA = muon.
  ...
  double zher[7] = {0.234, 0.303, 0.373, 0.447, 0.523, 0.593, 0.663};
  double Col[7] = {0.0};
  double Colerr[7] = {0.0};
  int nxb[7] = {0};
  double STxb[7] = {0};
  ...
  for (int j = 0; j<7; j++){
    ...
    //cout << Col[j] << endl;
       if (out.is_open()){
           out << zher[j] << " " << Col[j]/(STxb[j]) << " " <<sqrt((Colerr[j]-pow(Col[j],2))/nxb[j])/(STxb[j]) << endl;
       }
   }
