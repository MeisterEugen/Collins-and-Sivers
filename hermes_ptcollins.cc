...
int main(int argc, char* argv[]) {
  ...
  file_name.append("HERMES/ptCollins_");
  ...
  double pther[7] = {0.110, 0.211, 0.29, 0.37, 0.488, 0.669, 0.954};
  ...
  for (int j = 0; j<7; j++){
    ...
    //cout << Col[j] << endl;
       if (out.is_open()){
           out << pther[j] << " " << Col[j]/(DNNxb[j]*STxb[j]) << " " <<sqrt((Colerr[j]-pow(Col[j],2))/nxb[j])/(DNNxb[j]*STxb[j]) << endl;
       }
   }
