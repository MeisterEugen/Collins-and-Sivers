...
int main(int argc, char* argv[]) {
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    for (int i = 0; i < event.size(); ++i){
      ...
      if ( idHad== pid && zh>0.2 && sqrt(pT2)>0.1 && abs(statusHad)==83 ) {
        ...
        if (x>0.21 and x <= 1){
          Col[8] += 2.0*sin(2*phiHad);
          Colerr[8] += pow(2.0*sin(2*phiHad),2);
          nxb[8] += 1;
          DNNxb[8] += (1.0-dis.y);
          SLxb[8] += SL;
        }
      /*  Acoll[0] += 2.0*sin(phiHad);
        Acoll[1] += pow(2.0*sin(phiHad),2);
        Npi += 1;
  ...
  for (int j = 0; j<9; j++){
    ...
    //cout << Col[j] << endl;
       if (out.is_open()){
           out << xexp[j] << " " << (Col[j]*DNNxb[j])/(SLxb[j]) << " " <<sqrt((Colerr[j]-pow(Col[j],2))/nxb[j])*DNNxb[j]/(SLxb[j]) << endl;
       }
   }
