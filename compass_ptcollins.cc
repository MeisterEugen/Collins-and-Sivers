...
int main(int argc, char* argv[]) {
  ...
  double Q2min	  = 1.0;
  double nEvent	  = (int)std::stod(argv[2]);
  ...
  double ptexp[9] = {0.1545, 0.252, 0.3497, 0.4483, 0.5475, 0.668, 0.8168, 1.042, 1.554};
  double Col[9] = {0.0};
  double Colerr[9] = {0.0};
  int nxb[9] = {0};
  double DNNxb[9] = {0};
  ...
  for (int j = 0; j<9; j++){
    ...
    //cout << Col[j] << endl;
       if (out.is_open()){
           out << ptexp[j] << " " << Col[j]/(DNNxb[j]*STxb[j]) << " " <<sqrt((Colerr[j]-pow(Col[j],2))/nxb[j])/(DNNxb[j]*STxb[j]) << endl;
       }
   }
